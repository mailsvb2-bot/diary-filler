...
"%ISCC%" /DMyAppVersion=1.4.5 "installer\MedicalDiaryAutofill.iss"
if errorlevel 1 exit /b 1

if not exist "dist\MedicalDiaryAutofill-Setup-1.4.5.exe" (
  echo [ОШИБКА] Installer не создан.
  exit /b 1
)

echo ГОТОВО: dist\MedicalDiaryAutofill-Setup-1.4.5.exe
exit /b 0
