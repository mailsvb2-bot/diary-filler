...
"%ISCC%" /DMyAppVersion=1.4.6 "installer\MedicalDiaryAutofill.iss"
if errorlevel 1 exit /b 1

if not exist "dist\MedicalDiaryAutofill-Setup-1.4.6.exe" (
  echo [ОШИБКА] Installer не создан.
  exit /b 1
)

echo ГОТОВО: dist\MedicalDiaryAutofill-Setup-1.4.6.exe
exit /b 0
