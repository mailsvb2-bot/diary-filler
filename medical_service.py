...
все отмеченные в UI документы.
    ...
    def _validate_and_normalize_selected_data(self, data: PatientData, selected: Sequence[str]) -> None:
        ...
        # Selecting a specialized commission form is itself a positive routing
        # choice. An explicit contradictory shared decision must fail closed;
        # an empty decision is normalized to the selected form instead of
        # producing an internally inconsistent document set.
        ...
        # Psychiatric registration belongs to every medical document (diaries are
        # generated by a separate service and never enter this boundary).
        psych_decision = normalize_yes_no(data.psych_account_status)
        parsed_psych_decision, parsed_psych_year = parse_psych_account_value(data.psych_account)
        if not psych_decision:
            psych_decision = parsed_psych_decision
        if not psych_decision:
            raise ValueError("Укажите, состоит ли пациент на учёте у психиатров.")
        data.psych_account_status = psych_decision
        if psych_decision == "да":
            year = (data.psych_account_since_year or parsed_psych_year).strip()
            if not re.fullmatch(r"(?:19|20)\d{2}", year):
                raise ValueError("Укажите год постановки на учёт у психиатров в формате ГГГГ.")
        ...
        data.psych_account = format_psych_account_value(
        ...
        if selected_set & rvk_referral_docs:
            rvk_decision = normalize_yes_no(data.rvk_referral_present)
            parsed_rvk_decision, parsed_rvk_area = parse_rvk_referral_value(data.rvk_referral)
            if not rvk_decision:
                rvk_decision = parsed_rvk_decision
            if not rvk_decision:
                raise ValueError("Укажите, пациент по направлению из РВК или нет.")
            data.rvk_referral_present = rvk_decision
            if rvk_decision == "да":
                area = (data.rvk_referral_commissariat or parsed_rvk_area or data.rvk_military_commissariat).strip()
                if not area:
                    raise ValueError("Укажите район РВК для направления.")
                data.rvk_referral_commissariat = area
            else:
                data.rvk_referral_commissariat = ""
            data.rvk_referral = format_rvk_referral_decision(
                data.rvk_referral_present, data.rvk_referral_commissariat, data.rvk_referral
            )
