...
class DialogExpertMixin:
    ...
    def _prompt_common_output_requirements(self, *, include_discharge_date: bool, include_case_number: bool = True) -> bool:
        ...
        Номер истории болезни показывается только для медицинских документов
        блока 03 и переиспользуется всеми следующими popup-окнами.
        """
        ...
        fields: list[str] = []
        if include_case_number and (detail_rows or self._case_number_missing()):
    ...
    def _prompt_discharge_output_requirements(self) -> bool:
        ...
        rows: list[tuple[str, str]] = []
        fields: list[str] = []
        if detail_rows or self._case_number_missing():
            rows.append(("Номер истории болезни", self._case_number_popup_default()))
            fields.append("case_number")
        rows.extend(detail_rows)
        fields.extend(detail_fields)
